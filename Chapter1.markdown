...
这个神奇的效果是通过 Angular 的依赖注入系统实现的。依赖注入让我们遵循这样一种开发风格：我们的类只请求它所需要的东西，而不创建依赖关系。

这个效果遵循一个被称为得墨忒耳定律的设计模式，也被称作最少知识原则。由于我们的 `HelloController` 的工作只是设置 greeting 模型的初试状态，这种模式会告诉你它不应担心其他的事情，例如 `$scope` 是如何创建的，或者在哪里可以找到它。
